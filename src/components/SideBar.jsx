...
import { HomeIcon,UserIcon,ArrowLeftStartOnRectangleIcon } from '@heroicons/react/24/outline'
...
function SideBar() {
  ...
  useEffect(() => {
    fetch("https://hey.mahdisharifi.dev/api/tweets/popular", {
      method: "get",
      headers: {
    ...
    <div className="w-full h-full overflow-hidden flex flex-col  ">
      ...
      <div className="my-2 border-b-[1px] border-gray-200">
        ...
        <div className="flex flex-col gap-2 mb-3">
          <SideBarMenuItem name={"Home"} icon={<HomeIcon className="size-6"/>} path={"/"}/>
          <SideBarMenuItem name={"Profile"} icon={<UserIcon className="size-6"/>} path={"/profile"}/>
          <SideBarMenuItem name={"Logout"} icon={<ArrowLeftStartOnRectangleIcon className="size-6"/>} path={"/logout"}/>
        </div>
      </div>
